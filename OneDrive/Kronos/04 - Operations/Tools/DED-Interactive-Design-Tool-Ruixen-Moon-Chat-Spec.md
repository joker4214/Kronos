# DED Interactive Design Tool — Component Spec (Ruixen Moon Chat)

Moved out of the Obsidian vault 2026-08-06 — this is a raw component-install spec (code + shadcn/Tailwind setup instructions), not a memory note, so it lives here with the other DED technical build files instead. Originally pasted from a UI component registry ("Ruixen") as a starting point for an AI-chat-style hero/input component.

Context: Jason is considering an interactive tool on the DED site that lets prospects play with building a landing page or product image for their own Shopify store (mockup/co-design, screenshot-able, "I helped design part of my store"). This component — a full-bleed background image with a centered chat input and quick-action chips (Generate Code, Launch App, Landing Page, Image Assets, etc.) — is one possible front-end starting point for that, styled like an AI website-builder (v0.dev/bolt.new-style), not a finished spec for the actual interactive builder logic.

Not yet built or approved for the live site. See [[AI Business Empire]] / [[Dharma's Esthetic Design]] in the vault for the current discussion and decision.
